...
from app.schemas.data_source import DataSourceCreate, DocumentUploadResponse, RAGQueryResponse, RAGChunk
...
class RAGService:
    ...
    async def create_data_source(
        self, organization_id: UUID, data: DataSourceCreate
    ) -> DataSource:
    ...
    async def list_data_sources(self, organization_id: UUID) -> List[DataSource]:
    ...
    async def delete_data_source(
        ...
        data_source = await self.get_data_source(data_source_id, organization_id)
        if not data_source:
            return False
        ...
        for doc in documents:
            await self.embeddings.delete_document_chunks(
                str(organization_id), str(doc.id)
            )
    ...
    async def upload_document(
        ...
        storage_dir = os.path.join(settings.UPLOAD_DIR, str(organization_id))
    ...
    async def list_documents(
        self, organization_id: UUID, data_source_id: UUID = None
    ) -> List[Document]:
        """List documents for an organization"""
        query = select(Document).where(Document.organization_id == organization_id)

        if data_source_id:
            query = query.where(Document.data_source_id == data_source_id)
    ...
    async def delete_document(
        ...
        await self.embeddings.delete_document_chunks(
            str(organization_id), str(document_id)
        )
    ...
    async def query(
        ...
        query: str,
        top_k: int = 5,
        data_source_ids: List[UUID] = None,
    ) -> RAGQueryResponse:
        """Query the RAG system"""
        data_source_id_strs = (
            [str(ds_id) for ds_id in data_source_ids] if data_source_ids else None
        )
        ...
        results = await self.embeddings.search(
            ...
            query,
            top_k=top_k,
            data_source_ids=data_source_id_strs,
        )
